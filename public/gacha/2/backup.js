    ...
    r.catch(function(er) {
      ...
      r2.then(function(reply3) {
        ...
        //console.error("Couldn't get sprite for Pokemon: " + reply3.varieties[0].pokemon.name)
          ...
          imageActiveReqs[idx] = undefined
          console.error("Unable to find species " + reply3.varieties[0].pokemon.name + " (" + dat[0] + ")")
        });
      })
      r2.catch(function(er2) {
        ...
        imagecache[imageActiveReqs[idx][2]].img = image_unavailable2
        imageActiveReqs[idx] = undefined
        console.error("Unable to find species " + dat[0])
      });
    });
  } else {
...
function draw() {
  ...
  if (todayview) {
    ...
    if (calendar.length > 0) {
      ...
      calendarY = 0
      calendarH = (height - (showBottomBar ? 69 : 44) - (calendarRows - 1)*5) / calendarRows
      for (i = 0; calendarX < calendar[0][0]; i++, calendarX++) {
        var s2 = min(30, calendarH * 0.3)
        var DATE = getDayUniversal(displayMonth, 0, displayYear)
        DATE = new Date(DATE.getTime() - day_time * (calendar[0][0] - i - 1))
        var OFF = getCalendarOffset(DATE)
        var C = getDay(DATE, OFF)
        //boxText(C[3], 11 + calendarX*(calendarW + 5), 40 + calendarY*(calendarH + 5), 15, 18)
        if (legendaryImages[C[1]] != undefined) {
          var sz = min(calendarW, calendarH) - 16
          image(legendaryImages[C[1]], 10 + calendarX*(calendarW + 5) + calendarW/2 - sz/2, 38 + calendarY*(calendarH + 5) + calendarH/2 - sz/2, sz, sz)
        }
        //boxText(C[1], 11 + calendarX*(calendarW + 5), 38 + calendarY*(calendarH + 5) + calendarH - s2, calendarW - 2, s2)
      }
      ...
      for (var i = 0; i < calendar.length; i++) {
        ...
        /* Insert a second / to enable this block
        if (todayIndex != -1 && i == todayIndex + 1 && tomorrowRus == undefined) {
          tomorrowRus = guessPokerus(new Date(new Date(calendar[i][4], calendar[i][2], i+2 + pokerusOffset).getTime() + todayOffset*day_time))
        }
        //*/
        rect(10 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5), calendarW, calendarH)
        pop()
        if (legendaryImages[calendar[i][1]] != undefined) {
          //console.log(calendar[i][1])
          var sz = min(calendarW, calendarH) - 4
          image(legendaryImages[calendar[i][1]], 8 + calendarX*(calendarW + 5) + calendarW - sz, 41 + calendarY*(calendarH + 5) + max(0, min(8, calendarH - sz - 20)), sz, sz)
        }
        push()
        noStroke()
        //fill(calendar[i][6] % 2 == 0 ? color(255, 255, 255, 190) : color(201, 218, 245, 190))
        fill(255, 190)
        rect(11 + calendarX*(calendarW + 5), 40 + calendarY*(calendarH + 5), 15, 18)
        //fill(255, 190)
        var s2 = min(30, calendarH * 0.3)
        push()
        textSize(15)
        var imageSpacing = 0
        while (s2 > 20 && min(1, (calendarW - s2*3 - imageSpacing*2 - 4) / textWidth(calendar[i][1]))*12 < 10) {
          s2--
        }
        while (imageSpacing > -4 && min(1, (calendarW - s2*3 - imageSpacing*2 - 4) / textWidth(calendar[i][1]))*12 < 10) {
          imageSpacing--
        }
        while (s2 > 10 && min(1, (calendarW - s2*3 - imageSpacing*2 - 4) / textWidth(calendar[i][1]))*12 < 10) {
          s2--
        }
        var s4 = 25
        if (inRectArea(10 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5), calendarW, calendarH) && pkrs) {
          s4 = min(50, (calendarW - 4)/3.5)
        }
        var s3 = s2
        pop()
        if (showCalendarCycles) fill(calendar[i][6] % 2 == 0 ? color(250, 212, 212, 190) : color(201, 218, 245, 190))
        rect(11 + calendarX*(calendarW + 5), 38 + calendarY*(calendarH + 5) + calendarH - s4, calendarW - 2, s4)
        fill(255, 190)
        pop()
        boxText(calendar[i][3], 11 + calendarX*(calendarW + 5), 40 + calendarY*(calendarH + 5), 15, 18)
        textSize(15)
        if (inRectArea(10 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5), calendarW, calendarH) && pkrs) {
          //
        } else {
          boxText(calendar[i][1], 11 + calendarX*(calendarW + 5), 38 + calendarY*(calendarH + 5) + calendarH - 25, calendarW - 2 + (pkrs && showMiniSprites ? s3*-3 - 2 - imageSpacing*2 : 0), 25, pkrs && showMiniSprites)
        }
        textSize(10)
        var pk;
        pk = calendar[i][5]
        if (pkrs) {
          var popout = 10
          if (pk == undefined) pk = guessPokerus(new Date(new Date(calendar[i][4], calendar[i][2], i+1 + pokerusOffset).getTime() + todayOffset*day_time))
          if (pk.toString() == todayRus.toString() && todayIndex != -1 && displayMonth == month() - 1 && displayYear == year()) {
            //if (todayIndex < i) todayOffset++
            //if (todayIndex > i) todayOffset--
            //if (pk == undefined) pk = guessPokerus(new Date(new Date(calendar[i][4], calendar[i][2], calendar[i][3]).getTime() + todayOffset*day_time))
          }
          if (inRectArea(10 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5), calendarW, calendarH) && pkrs) {
            var IS = imageSpacing
            s3 = imageSpacing
            imageSpacing = s4
            getSprite(pk[0], 10 + calendarX*(calendarW + 5) + calendarW/2 - (imageSpacing + popout)/2 - imageSpacing, 39 + calendarY*(calendarH + 5) + calendarH - imageSpacing - popout/2, imageSpacing + popout, imageSpacing + popout)
            getSprite(pk[1], 10 + calendarX*(calendarW + 5) + calendarW/2 - (imageSpacing + popout)/2, 39 + calendarY*(calendarH + 5) + calendarH - imageSpacing - popout/2, imageSpacing + popout, imageSpacing + popout)
            getSprite(pk[2], 10 + calendarX*(calendarW + 5) + calendarW/2 - (imageSpacing + popout)/2 + imageSpacing, 39 + calendarY*(calendarH + 5) + calendarH - imageSpacing - popout/2, imageSpacing + popout, imageSpacing + popout)
            imageSpacing = IS
          } else if (showMiniSprites) {
            getSprite(pk[0], 10 + calendarX*(calendarW + 5) + calendarW - s3*3 - popout/2 - imageSpacing*2, 39 + calendarY*(calendarH + 5) + calendarH - s3 - popout/2, s3 + popout, s3 + popout)
            getSprite(pk[1], 10 + calendarX*(calendarW + 5) + calendarW - s3*2 - popout/2 - imageSpacing*1, 39 + calendarY*(calendarH + 5) + calendarH - s3 - popout/2, s3 + popout, s3 + popout)
            getSprite(pk[2], 10 + calendarX*(calendarW + 5) + calendarW - s3*1 - popout/2 - imageSpacing*0, 39 + calendarY*(calendarH + 5) + calendarH - s2/2 - s3/2 - popout/2, s3 + popout, s3 + popout)
          }
        }
        if ((inRectArea(10 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5), calendarW, calendarH) && pkrs) || showall) {
          if (pk == undefined) pk = guessPokerus(new Date(new Date(calendar[i][4], calendar[i][2], i+1 + pokerusOffset).getTime() + todayOffset*day_time))
          push()
          pkrsNames = [
            genSpeciesNamesMerged[genSpeciesMerged.indexOf(pk[0])],
            genSpeciesNamesMerged[genSpeciesMerged.indexOf(pk[1])],
            genSpeciesNamesMerged[genSpeciesMerged.indexOf(pk[2])]
          ]
          pkrsNames.sort(function(a, b) {
            return (a[1]+a[2])*1 - (b[1]+b[2])*1
          })
          var rusBoxY = s2 - s4
          pkrsNames = pkrsNames.map(v => v.substring(5))
          fill(255, 190)
          stroke(0, 150)
          var rusBoxW = max(textWidth("Pokérus")+3, textWidth(pkrsNames[0]), textWidth(pkrsNames[1]), textWidth(pkrsNames[2])) + 6
          rect(12 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - textLeading()*4.2 + rusBoxY, rusBoxW, textLeading()*4.2)
          line(12 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - textLeading()*3 - 2 + rusBoxY, 12 + calendarX*(calendarW + 5) + rusBoxW, 39 + calendarY*(calendarH + 5) + calendarH - s2 - textLeading()*3 - 2 + rusBoxY)
          pop()
          text("Pokérus", 12 + rusBoxW/2 - textWidth("Pokérus")/2 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - 5 - textLeading()*3 + rusBoxY)
          text(pkrsNames[0], 15 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - 4 - textLeading()*2 + rusBoxY)
          text(pkrsNames[1], 15 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - 5 - textLeading()*1 + rusBoxY)
          text(pkrsNames[2], 15 + calendarX*(calendarW + 5), 39 + calendarY*(calendarH + 5) + calendarH - s2 - 5 - textLeading()*0 + rusBoxY)
        }
        textSize(12)
        // Date Box
        line(26 + calendarX*(calendarW + 5), 40 + calendarY*(calendarH + 5), 26 + calendarX*(calendarW + 5), 58 + calendarY*(calendarH + 5))
        line(11 + calendarX*(calendarW + 5), 58 + calendarY*(calendarH + 5), 26 + calendarX*(calendarW + 5), 58 + calendarY*(calendarH + 5))
        // Image
        //lineRect(8 + calendarX*(calendarW + 5) + calendarW - sz, 41 + calendarY*(calendarH + 5) + max(0, min(8, calendarH - sz - 20)), sz, sz, false, true, false, false)
        // Pokerus sprites
        line(10 + calendarX*(calendarW + 5), 39 + calendarH - s3 + calendarY*(calendarH + 5), 10 + calendarW + calendarX*(calendarW + 5), 39 + calendarH - s3 + calendarY*(calendarH + 5))
        // End calendar drawing loop
      }
      var initCalendarX = calendarX
      calendarX++
      for (i = 0; calendarX < 7; i++, calendarX++) {
        var s2 = min(30, calendarH * 0.3)
        var C = getDay(displayYear, displayMonth+1, i+1)
        //boxText(C[3], 11 + calendarX*(calendarW + 5), 40 + calendarY*(calendarH + 5), 15, 18)
        if (legendaryImages[C[1]] != undefined) {
          var sz = min(calendarW, calendarH) - 16
          image(legendaryImages[C[1]], 10 + calendarX*(calendarW + 5) + calendarW/2 - sz/2, 38 + calendarY*(calendarH + 5) + calendarH/2 - sz/2, sz, sz)
    ...
    if (showBottomBar) {
      ...
      if (inRectArea(20 + (width - 40)/3, height - 25, width - 35 - (width - 40)/3 - (width - 40)/6, 20) && !click) {
        ...
        boxText(timedisplay + " - click here to save a photo", 20 + (width - 20)/3, height - 25, width - 35 - (width - 20)/3 - (width - 20)/6, 20)
        ...
        boxText(timedisplay, 20 + (width - 40)/3, height - 25, width - 35 - (width - 40)/3 - (width - 20)/6, 20)
      }
      if (takeScreenshot) {
