...
<div class="wiki-page">
    ...
    <form method="POST" action="/build/{{ $build->slug }}/edit.html" id="buildForm">
        @csrf

        {{-- Title + description --}}
        <div class="build-form-section">
            <label class="build-form-label" for="title">Build Title <span class="req">*</span></label>
            <input type="text" id="title" name="title" value="{{ old('title', $build->title) }}"
        ...
        <div class="build-form-submit-row">
            ...
            <a href="/build/{{ $build->slug }}.html" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
...
// ── Init ──────────────────────────────────────────────────────────
// Restore from old() on validation failure, otherwise load existing build team
@if(old('team'))
