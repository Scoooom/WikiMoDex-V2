{{--
    Shared build form partial.
    Variables:
      $action      — form POST action URL
      $submitLabel — submit button text
      $cancelUrl   — cancel link URL
      $build       — existing CommunityBuild (null for create)
      $items       — GameItem collection (whitelisted)
--}}
...
<form method="POST" action="{{ $action }}" id="buildForm">
    @csrf
    ...
    <div class="build-form-section">
        ...
        <textarea id="description" name="description" class="build-form-textarea"
                  ...
                  maxlength="1000" rows="3">{{ old('description', $build->description ?? '') }}</textarea>
    </div>
...
function addItemRow(slot, nameVal, keyVal, stackVal, params) {
    ...
    function render(key, currentName, p) {
        ...
        makeTypeahead(nameInput, null, r => {
            ...
            if (r.key !== prevKey) render(r.key, r.value, {});
        }, ITEM_LIST);
    }
...
function makeTypeahead(input, fetchUrl, onSelect, localData) {
    ...
    input.addEventListener('input', () => {
        ...
        if (localData) { render(filterLocal(q)); return; }
...
// ── Init ──────────────────────────────────────────────────────────
@if(old('team'))
    @foreach(old('team', []) as $slot)
    addSlot({!! json_encode($slot) !!});
    @endforeach
@elseif(isset($build) && $build?->team)
    @foreach($build->team as $slot)
    addSlot({!! json_encode($slot) !!});
    @endforeach
@else
    addSlot();
